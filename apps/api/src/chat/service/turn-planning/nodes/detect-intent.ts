...
const ConversationIntentModelOutputSchema =
  ConversationIntentSchema.partial().extend({
    secondary: z
      .union([
...
function repairConversationIntentModelOutput(
  ...
  const shouldClarify =
    intent.shouldClarify ?? intent.requestedAgentAction === "ask_follow_up";
  const secondary =
...
export function createDetectIntentNode(env: ParsedApiEnvBindings) {
  ...
  return async (
    ...
    const intent = await invokeStructuredOutputWithFallback({
      cacheKey: `${baseURL}|${modelName}`,
      operation: "conversation intent",
      invoke: (method) =>
        invokeConversationIntentAnalysis(model, method, state),
      fallback: FALLBACK_CONVERSATION_INTENT,
    });
